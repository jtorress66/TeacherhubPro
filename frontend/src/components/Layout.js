...
const Layout = ({ children }) => {
  ...
  const navItems = [
    ...
    { path: '/teacher/play-to-learn', icon: Rocket, label: language === 'es' ? 'Jugar y Aprender' : 'Play to Learn', gradient: 'from-indigo-500 to-purple-600', isNew: true },
    ...
    { path: '/gradebook', icon: BookOpen, label: language === 'es' ? 'Tareas y Calificaciones' : 'Assignments & Gradebook', gradient: 'from-rose-400 to-rose-600' },
    { path: '/ai-grading', icon: Brain, label: language === 'es' ? 'Calificación IA' : 'AI Grading', gradient: 'from-violet-400 to-fuchsia-600', isNew: true },
    { path: '/gradebook/reports', icon: BarChart3, label: language === 'es' ? 'Rep. Notas' : 'Grade Report', gradient: 'from-pink-400 to-pink-600' },
    { path: '/report-cards', icon: GraduationCap, label: language === 'es' ? 'Notas' : 'Report Cards', gradient: 'from-emerald-400 to-green-600', isNew: true },
